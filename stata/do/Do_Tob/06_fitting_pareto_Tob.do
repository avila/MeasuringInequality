...
* 1. WV: within-variance (see formula x.x on page xx) 
	...
	forval imp=1(1)5 {
		...
		*generate weighted mean of ln_netwealth
		...
		scalar sc_w_mean_ln_nw_p95_sp_`imp' = (W_sp * ln_nw_`imp'_)/sc_N_`imp'_sp_
		
		*estimate within variance (= SSE / sum(((W*ln(nw)) - (sum( W*ln(nw) / sum(W) )))^2)	
		estimates restore reg_`imp'_p95_sp
		qui scalar sc_wv_p95_sp_`imp' = e(rss) / sum((sum(W_sp * ln_nw_`imp'_)/sc_w_mean_ln_nw_p95_sp_`imp')^2)
		
		if "$show_all" == "TRUE" {
		di "within variance: imp 1: `=sc_wv_p95_sp_1'; imp2: `=sc_wv_p95_sp_2'; imp3: `=sc_wv_p95_sp_3'; imp4: `=sc_wv_p95_sp_4'; imp5: `=sc_wv_p95_sp_5'" 
		}
