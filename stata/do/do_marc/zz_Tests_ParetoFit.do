...
foreach dat of local data {
	forval imp=1(1)`=m' {
		foreach pc of local pctile {
			...
			if "`dat'" == "sp" {
				paretofit _`imp'_nw if D_pretest == 0 [fweight=round(W_`dat',)], x0(`=sc_thres_`imp'_`pc'_sp') cdf(cdf_`imp'_`pc'_`dat') robust
				scalar sc_alpha_pfit_`imp'_`pc'_`dat' = e(ba)
			}
			if "`dat'" == "pt" {
				paretofit _`imp'_nw if D_pretest == 1 [fweight=W_`dat'], x0(`=sc_thres_`imp'_`pc'_sp') cdf(cdf_`imp'_`pc'_`dat') robust
				scalar sc_alpha_pfit_`imp'_`pc'_`dat' = e(ba)
			}
		}
	}
