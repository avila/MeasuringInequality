...
/* Note: Here, pareto alphas are estimated on the basis of the SOEP and the 
	...
	5x pretest with p95 thres,  5x pretest with p95 thres.
	...
	3.	Test equality of Pareto's Alpha for SOEP vs. Pretest
			...
			3.2. Hausman (suest)
			3.3. chow test
	4.	Test equality of Pareto's Alpha for SOEP (Pretest) across pct
			4.1. ttest
			4.2. Hausman (suest)
	...
	foreach type in sp pt sppt {
		...
		foreach pct of local pctile {
			...
			if "`type'" == "sp" {
				...
				if "$show_all" == "TRUE" {
					di "sc_alpha_`imp'_`pct'_`type' = `=sc_alpha_`imp'_`pct'_`type''"
					di "sc_alpha_`imp'_`pct'_`type' = `=sc_alpha_`imp'_`pct'_`type''"
					di "N: sc_alpha_`imp'_`pct'_`type' = `=sc_alpha_`imp'_`pct'_`type''"
					di "r2: sc_alpha_`imp'_`pct'_`type' = `=sc_alpha_`imp'_`pct'_`type''"
				}
		}
	}
}
...
* display all produced estimates
estimates dir
...
*		4.	Test equality of Pareto's Alpha for SOEP (Pretest) across pct
*		
*			alpha_imp1_p95 = alpha_imp1=p99, alpha_imp2_p95 = alpha_imp2=p99, ...
*
*				4.1. ttest, 4.2. Hausman (suest)
*		
********************************************************************************
...
foreach dat of local data {
	forval imp=1(1)`=m' {
		di _newline in red "+++++ ttest: ln_nw_`imp'_`dat'_p99 = ln_nw_`imp'_`dat'_p95 +++++" 
		di in red "+++++ threshold: `dat', imputation: `imp' +++++"
		ttest [reg_`imp'_p99_`dat'_mean]ln_nw_`imp'_=[reg_`imp'_p95_`dat'_mean]ln_nw_`imp'_
	}
}

********************************************************************************
*	4.2. Hausman-test suest (seemingly unrelated regressions)
********************************************************************************
* run suest, Hausman-test equality of alphas
foreach dat of local data {
	forval imp=1(1)5 {
		* test SOEP vs. Pretest
		test [reg_`imp'_p99_`dat'_mean]ln_nw_`imp'_=[reg_`imp'_p95_`dat'_mean]ln_nw_`imp'_
		scalar sc_testpval_`imp'_`dat'_p95vsp99 = r(p)
	}
}
